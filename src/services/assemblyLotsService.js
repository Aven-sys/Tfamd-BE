import crypto from 'crypto';
import { query, getClient } from '../config/database.js';
import { readJsonFile } from '../utils/fileUtils.js';

/**
 * Assembly Lots Service - Inserts assembly lots data into PostgreSQL
 */
class AssemblyLotsService {
    /**
     * Generate a unique UUID
     */
    static generateUUID() {
        return crypto.randomUUID();
    }

    /**
     * Insert multiple records using batch insert with transaction
     */
    static async insertMany(records, batchSize = 500) {
        const client = await getClient();
        let insertedCount = 0;
        const startTime = Date.now();
        
        try {
            await client.query('BEGIN');
            
            const totalBatches = Math.ceil(records.length / batchSize);
            
            for (let i = 0; i < records.length; i += batchSize) {
                const batch = records.slice(i, i + batchSize);
                const values = [];
                const placeholders = [];
                
                batch.forEach((record, index) => {
                    const offset = index * 11;
                    placeholders.push(`($${offset + 1}, $${offset + 2}, $${offset + 3}, $${offset + 4}, $${offset + 5}, $${offset + 6}, $${offset + 7}, $${offset + 8}, $${offset + 9}, $${offset + 10}, $${offset + 11})`);
                    values.push(
                        this.generateUUID(),
                        record.assyMotherLot || '',
                        record.device ?? 0,
                        record.operationStep ?? 0,
                        record.eqpid || '',
                        record.trackInTime || null,
                        record.trackOutTime || null,
                        record.inQty ?? 0,
                        record.outQty ?? 0,
                        record.operator || '',
                        record.status || ''
                    );
                });
                
                const insertQuery = `
                    INSERT INTO assembly_mother_lots 
                    (id, assy_mother_lot, device, operation_step, eqp_id, track_in_time, track_out_time, in_qty, out_qty, operator, status)
                    VALUES ${placeholders.join(', ')}
                `;
                
                const result = await client.query(insertQuery, values);
                insertedCount += result.rowCount;
                
                const batchNum = Math.floor(i / batchSize) + 1;
                const percent = Math.round((batchNum / totalBatches) * 100);
                console.log(`📥 Batch ${batchNum}/${totalBatches} (${percent}%): Inserted ${result.rowCount} records`);
            }
            
            await client.query('COMMIT');
            const duration = ((Date.now() - startTime) / 1000).toFixed(2);
            console.log(`✅ Transaction committed in ${duration}s`);
            
            return { total: records.length, inserted: insertedCount, duration: parseFloat(duration) };
        } catch (error) {
            await client.query('ROLLBACK');
            console.error('❌ Transaction rolled back:', error.message);
            throw error;
        } finally {
            client.release();
            console.log('🔄 Client released back to pool');
        }
    }

    /**
     * Process JSON file and insert into assembly_mother_lots (batch mode)
     */
    static async processAssemblyLotsFile(filePath, batchSize = 500) {
        // Check if records already exist in database
        const existingCount = await this.getCount();
        if (existingCount > 5) {
            console.log(`ℹ️ Database already has ${existingCount} records. Skipping insert.`);
            return { total: 0, inserted: 0, skipped: true, duration: 0 };
        }

        const jsonData = await readJsonFile(filePath);
        return await this.insertMany(jsonData, batchSize);
    }

    /**
     * Get total count of records in assembly_mother_lots
     */
    static async getCount() {
        const result = await query('SELECT COUNT(*) as total FROM assembly_mother_lots');
        return parseInt(result.rows[0].total);
    }
}

export default AssemblyLotsService;
